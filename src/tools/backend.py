...
class BackendError(Exception):
    """Ошибка учебного действия с полем, которое нужно уточнить у клиента."""
...
class MockBackend:
    """Операции над копией датасета в памяти; отдельный экземпляр на разговор."""
        ...
        for policy in self.data["policies"]:
            if policy["policy_number"] == number and policy["client_id"] == client_id:
                ...
                result["status"] = (
                    ...
                    if today > result["end_date"]
                    else "active"
                )
                if active and result["status"] != "active":
                    raise BackendError(
                        "policy_inactive", "Полис сейчас не действует.", "policy_number"
                    )
                return result
        raise BackendError(
            "not_found", "Полис не найден среди полисов этого клиента.", "policy_number"
        )
    ...
    def execute(
        self, name: str, arguments: dict, client_id: str | None = None, *, confirmed: bool = False
    ) -> dict:
        ...
        self.events.append(
            {"action": name, "arguments": deepcopy(arguments), "result": deepcopy(result)}
        )
        return result
    ...
    def _dispatch(self, name: str, args: dict, client_id: str | None, *, commit: bool) -> dict:
        ...
        for required in self.catalog.actions[name]["inputs"]:
            if not any(args.get(option) not in (None, "", []) for option in required.split("|")):
                raise BackendError(
                    "invalid_input", "Не хватает данных для действия.", required.split("|")[0]
                )
        if name == "find_client":
            for client in self.data["clients"]:
            ...
            raise BackendError(
                "not_found",
                "Не удалось найти клиента по указанным данным.",
        ...
        if name == "get_policy":
            ...
            if not number:
                found = [
                    p
                    for p in self.policies(client_id)
        ...
        if name == "get_bm_class":
            ...
            value = next(
                (c["bm_class"] for c in self.data["clients"] if c["iin"] == iin),
        ...
        if name == "create_claim":
            ...
            if (
                incident_date > self.catalog.today.isoformat()
                or not policy["start_date"] <= incident_date <= policy["end_date"]
            ):
                raise BackendError(
                    "policy_inactive", "Дата события вне срока действия полиса.", "incident_date"
                )
            if policy["product"] != args["product_type"]:
                raise BackendError(
                    "invalid_input", "Тип полиса не совпадает с заявлением.", "policy_number"
                )
            result = {
                "policy_number": policy["policy_number"],
                "incident_date": incident_date,
                "incident_description": args["incident_description"],
                "claim_type": args["product_type"],
            }
            if commit:
        ...
        if name == "update_contact":
            ...
            if field not in {"phone", "email", "address"}:
                raise BackendError(
                    "invalid_input", "Недопустимое контактное поле.", "contact_field"
                )
            try:
                value = (
                    self.catalog.normalize(field, args["new_value"])
                    if field != "address"
            ...
            if field == "phone" and any(
                c["phone"] == value and c["client_id"] != client_id for c in self.data["clients"]
            ):
                raise BackendError(
                    "invalid_input", "Этот телефон уже привязан к другому клиенту.", "new_value"
                )
            if commit:
        ...
        if name == "check_payment":
            payments = [
                ...
                if p["client_id"] == client_id and p["date"] == args["payment_date"]
            ]
            if not payments:
    ...
    def _quote(self, name: str, args: dict) -> dict:
        ...
        if name == "calc_ogpo_price":
            ...
            classes = [
                self._dispatch("get_bm_class", {"iin": i}, None, commit=False)["bm_class"]
                for i in args["drivers_iin"]
            ]
            coef = max(rules["bm_coef"][c] for c in classes)
            price = (
                rules["base_by_region_kzt"][args["region"]]
                * rules["vehicle_type_coef"][args["vehicle_type"]]
                * coef
            )
            ...
            price = (
                args["car_value"]
                * rules["rate_by_car_age"][band]
                * rules["franchise_coef"][str(args["franchise"])]
            )
        else:
            ...
            if price is None:
                raise BackendError(
                    "invalid_input", "Для этого продукта нет такой страховой суммы.", "sum_insured"
                )
    ...
    def _book(self, name: str, args: dict, client_id: str | None, *, commit: bool) -> dict:
        ...
        if requested < self.catalog.today:
            raise BackendError(
                "invalid_input", "Выберите сегодняшнюю или будущую дату.", "preferred_date"
            )
        if name == "book_appointment":
            ...
            if requested.isoformat() > policy["end_date"]:
                raise BackendError(
                    "policy_inactive", "Полис истекает раньше даты приёма.", "preferred_date"
                )
            specialty = args["doctor_specialty"]
            if specialty != "therapist" and policy["details"]["package"] == "Basic":
                raise BackendError(
                    "not_covered",
                    "В Basic для специалиста нужно направление; его проверит оператор.",
                )
            choices = [
                c
                for c in self.kb["clinics"]
                if c["city"] == args["city"] and specialty in c["specialties"]
            ]
            if not choices:
                raise BackendError(
                    "no_availability", "Подходящей клиники в этом городе нет.", "city"
                )
            place = choices[0]
        else:
            self.claim(args["claim_number"], client_id)
            place = next(
                (p for p in self.kb["inspection_points"] if p["city"] == args["city"]),
        ...
        if requested.weekday() >= 5:
            raise BackendError(
                "no_availability", "В учебном расписании доступны рабочие дни.", "preferred_date"
            )
        hours = (10, 11, 14, 16)
        occupied = {
            a["slot_datetime"] for a in self.appointments if a["address"] == place["address"]
        }
        slot = next(
            (
                datetime.combine(requested, time(hour=h)).isoformat(timespec="minutes")
                for h in hours
                if datetime.combine(requested, time(hour=h)).isoformat(timespec="minutes")
                not in occupied
            ),
            None,
        )
        if slot is None:
            raise BackendError(
                "no_availability", "На этот день свободных мест нет.", "preferred_date"
            )
