...
from src.config import MAX_AUDIO_BYTES


class TelegramError(Exception):
    """Безопасное для журнала описание ошибки без URL, содержащего токен."""
...
class TelegramAPI:
    """Получает обновления и передаёт текст, голос и трассировки в Telegram."""
    ...
    def send_voice(self, chat_id: int, content: bytes):
        self.call(
            "sendVoice", {"chat_id": chat_id}, upload=("voice", "reply.mp3", content, "audio/mpeg")
        )

    def send_trace(self, chat_id: int, trace: dict):
        self.call(
            "sendDocument",
            {"chat_id": chat_id, "caption": "Трассировка последней реплики"},
            upload=(
                "document",
                "trace.json",
